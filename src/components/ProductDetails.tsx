...
export default function ProductDetails({ product }: { product: Product }) {
  ...
  const cartItems = useCartStore((state: any) => state.cartItems)  // Get cart items from the store
  ...
  const handleAddToCart = () => {
    if (isProductInCart) {
      ...
      addToCart({ ...product, size: selectedSize }); // Add selected size to the cart item
      setIsAdded(true);  // Change button state to "Added to Cart"
      toast.success(`${product.title} added to cart! 🛒`, {
        position: "top-center",
        duration: 3000,
      });
      setTimeout(() => setIsAdded(false), 2000); // Reset button state after a while
    }
  }
